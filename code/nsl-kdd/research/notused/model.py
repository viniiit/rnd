...
data_df = pd.DataFrame(X_test, columns= ["duration", "protocol_type", "service", "flag", "src_bytes", "dst_bytes", "land", "wrong_fragment", "urgent", "hot", "num_failed_logins", "logged_in", "num_compromised", "root_shell", "su_attempted", "num_root", "num_file_creations", "num_shells", "num_access_files", "num_outbound_cmds", "is_host_login", "is_guest_login", "count", "srv_count", "serror_rate", "srv_serror_rate", "rerror_rate", "srv_rerror_rate", "same_srv_rate", "diff_srv_rate", "srv_diff_host_rate", "dst_host_count", "dst_host_srv_count", "dst_host_same_srv_rate", "dst_host_diff_srv_rate", "dst_host_same_src_port_rate", "dst_host_srv_diff_host_rate", "dst_host_serror_rate", "dst_host_srv_serror_rate", "dst_host_rerror_rate", "dst_host_srv_rerror_rate"])
encoded_data_path = 'encoded_data.csv'
data_df.to_csv(encoded_data_path, index=False)

# Assuming 'encoder' is your fitted LabelEncoder
...
model = load_model('binary_model_full copy.h5')

print("Before saving the model:")
print("Weights Sample:", model.get_weights()[0][0][:10])  # Print the first ten weights of the first layer
...
model.save('binary_model_full.h5', include_optimizer=True)

print("After saving the model:")
print("Weights Sample:", model.get_weights()[0][0][:10])
...
# print(train_data['is_attack'])
# print(test_data['is_attack'])
...
X_main = train_data.drop(['class', 'difficulty', 'is_attack'], axis=1)
X_main_test = test_data.drop(['class', 'difficulty', 'is_attack'], axis=1)

# 'is_attack' feature alone
X_is_attack = train_data[['is_attack']]
X_is_attack_test = test_data[['is_attack']]

# Encode main categorical features
encoder1 = LabelEncoder()
for col in ['protocol_type', 'service', 'flag']:
    X_main[col] = encoder1.fit_transform(X_main[col])
    X_main_test[col] = encoder1.transform(X_main_test[col])



# Assuming 'encoder' is your fitted LabelEncoder
joblib.dump(encoder1, 'encoder2.joblib')
...
# Encode the labels
attack_encoder1 = LabelEncoder()
y_encoded = attack_encoder1.fit_transform(y)

# Add an "unknown" class if not already present
if 'unknown' not in attack_encoder1.classes_:
    classes = list(attack_encoder1.classes_) + ['unknown']
    attack_encoder1.classes_ = np.array(classes)

def transform_with_unknown_handling(encoder1, labels):
    unknown_class_index = np.where(encoder1.classes_ == 'unknown')[0][0]
    transformed_labels = []
    for label in labels:
        try:
            transformed_label = encoder1.transform([label])[0]
        except ValueError:
            # Assign the "unknown" class index for unseen labels
            transformed_label = unknown_class_index
        transformed_labels.append(transformed_label)
...
y_test_encoded = transform_with_unknown_handling(attack_encoder1, y_test)


# Assuming 'encoder' is your fitted LabelEncoder
joblib.dump(attack_encoder1, 'attack_encoder1.joblib')
...
model1 = build_mul_model(num_main_features, num_classes)
# model1 = load_model('multiclass_model_full copy.h5')

print("Before saving the model:")
print("Weights Sample:", model1.get_weights()[0][0][:10])  # Print the first ten weights of the first layer
...
model1.save('multiclass_model_full.h5', include_optimizer=True)

print("After saving the model:")
print("Weights Sample:", model1.get_weights()[0][0][:10])


mul_model_summary_str = []
model1.summary(print_fn=lambda x: mul_model_summary_str.append(x))
...
predictions=np.argmax(model1.predict([X_main_test, X_is_attack_test]),axis=-1)

cm = confusion_matrix(y_test_encoded, predictions) #attack=negative, normal=positive
print("Confusion Matrix:\n", cm)
fig1, ax3 = plt.subplots()
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=attack_encoder1.classes_, yticklabels=attack_encoder1.classes_)
ax3.set_xlabel('Predicted labels')
ax3.set_ylabel('True labels')
ax3.set_title('Confusion Matrix')
ax3.xaxis.set_ticklabels(attack_encoder1.classes_)
ax3.yaxis.set_ticklabels(attack_encoder1.classes_)
...
class_labels = attack_encoder1.classes_  # This should correspond to the labels used to encode y
...
adjusted_target_names = [name for i, name in enumerate(attack_encoder1.classes_) if i in np.unique(y_test_encoded)]
...
predicted_class_labels = attack_encoder1.inverse_transform(predictions)
actual_class_labels = attack_encoder1.inverse_transform(y_test_encoded)
...
# Evaluate the model on the test data
test_results = model1.evaluate([X_main_test, X_is_attack_test], y_test_encoded, verbose=1)
